...
interface Lesson {
  ...
  noi_dung: string;
  loai: string;
  ngay_tao?: string; // Optional vì bảng lesson có thể chưa có ngay_tao
}
...
function LessonDetail() {
    ...
    fetch(`http://localhost:8080/lesson/type/${loai}`, {
      ...
      .then((res) => {
        if (Array.isArray(res)) {
          const filtered = res
            .filter((lesson) => lesson.id !== currentId)
            .slice(0, 3); // Giới hạn 3 bài học
  ...
  const handleLike = () => {
    // TODO: Implement like functionality (e.g., call API to increment like count)
    alert('Đã thích bài học!');
  };

  const handleShare = () => {
    // TODO: Implement share functionality (e.g., copy URL to clipboard)
    ...
    return (
      <div className="bg-gray-900 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white text-lg">Đang tải bài học...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="bg-gray-900 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-4">{error || 'Không tìm thấy bài học'}</h2>
          <button
      ...
      <div className="bg-gradient-to-r from-gray-800 to-gray-900 shadow-lg">
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="flex items-center mb-4">
            <span
              className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium text-white ${categoryInfo.color} mr-4`}
            >
              <span className="mr-1">{categoryInfo.icon}</span>
              {categoryInfo.name}
            </span>
            <span className="text-gray-400 text-sm">{formatDate(lesson.ngay_tao)}</span>
          </div>

          <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">{lesson.tieu_de}</h1>

          <p className="text-xl text-gray-300 leading-8 max-w-4xl">{lesson.tom_tat}</p>
        </div>
      </div>
              ...
              <div className="prose prose-lg prose-invert max-w-none">
                <ReactMarkdown
                  components={{
                    ...
                    p: ({ children }) => <p className="text-gray-300 mb-4 leading-7">{children}</p>,
                    ul: ({ children }) => (
                    ...
                    em: ({ children }) => <em className="text-teal-200">{children}</em>,
                  }}
                >
                  {lesson.noi_dung}
                </ReactMarkdown>
                ...
                <div className="space-y-4">
                  {relatedLessons.map((related) => (
                    ...
                    >
                      {related.hinh_anh && (
                        <img
                          src={related.hinh_anh}
                          alt={related.tieu_de}
                          className="w-full h-32 object-cover rounded-lg mb-3"
                        />
                      )}
                      <h3 className="text-lg font-semibold text-white">{related.tieu_de}</h3>
                      <p className="text-gray-400 text-sm line-clamp-2">{related.tom_tat}</p>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-gray-400">Không có bài học liên quan.</p>
              )}
            </div>
          </div>
        </div>
