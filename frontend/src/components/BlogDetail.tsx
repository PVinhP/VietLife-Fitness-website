...
const BlogDetail = () => {
    ...
    useEffect(() => {
        const fetchBlog = async () => {
            try {
                const response = await fetch(`https://backend-rjhh.onrender.com/blogs/${id}`)
                if (!response.ok) {
    ...
    const fetchRelatedBlogs = async (category: string, currentBlogId: number) => {
        try {
            const response = await fetch(`https://backend-rjhh.onrender.com/blogs/category/${encodeURIComponent(category)}`)
            if (response.ok) {
                const data = await response.json()
    ...
    const getImageUrl = (hinh_anh: string) => {
        if (hinh_anh && hinh_anh.trim() !== '') {
            return hinh_anh.startsWith('http') ? hinh_anh : `https://backend-rjhh.onrender.com/uploads/${hinh_anh}`
        }
