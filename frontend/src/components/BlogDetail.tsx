...
const BlogDetail = () => {
    ...
    useEffect(() => {
        const fetchBlog = async () => {
            try {
                const response = await fetch(`https://vietlife-fitness-website-host.onrender.com/blogs/${id}`)
                if (!response.ok) {
    ...
    const fetchRelatedBlogs = async (category: string, currentBlogId: number) => {
        try {
            const response = await fetch(`https://vietlife-fitness-website-host.onrender.com/blogs/category/${encodeURIComponent(category)}`)
            if (response.ok) {
                const data = await response.json()
